...
public class OscillatingEnemyScript : MonoBehaviour, IDamageable {
	...
	void Update () {
		...
		transform.localPosition = new Vector3(Mathf.Sin(Mathf.PI * 2f * (Time.time + offset) / oscillateTime) * oscillateWidth, 0, transform.localPosition.z - (moveSpeed * Time.deltaTime));
	}
